...
struct input_event *normal_mode(struct input_event *start_ev, int oneshot)
{
	...
	while (1) {
		...
		if (!ev) {
			...
			int has_entry_pulse =
			    platform->has_active_entry_pulse &&
		...
		if (config_input_match_loose(ev, "scroll_down")) {
			...
			if (ev->pressed) {
				...
				if (platform->key_tap) {
					...
					uint8_t up_code =
					    platform->input_special_to_code(
						"uparrow");
					platform->key_tap(up_code,
							  PLATFORM_MOD_META);
				} else {
					int amount = config_get_int(
					    "scroll_home_amount");
					platform->scroll_amount(SCROLL_UP,
								amount);
				}
			}
			...
			if (ev->pressed) {
				...
				if (platform->key_tap) {
					...
					uint8_t down_code =
					    platform->input_special_to_code(
						"downarrow");
					platform->key_tap(down_code,
							  PLATFORM_MOD_META);
				} else {
					int amount = config_get_int(
					    "scroll_home_amount");
					platform->scroll_amount(SCROLL_DOWN,
								amount);
				}
			}
		...
		if (config_input_match(ev, "word_left")) {
			...
			if (nx > maxx)
				nx = maxx;
			move(scr, nx, my, !show_cursor, rapid_mode);
		} else if (config_input_match(ev, "word_up")) {
			int step = config_get_int("normal_large_step");
			int ny = my - step;
			const int miny = 1;
			const int maxy = sh - cursz;
			if (ny < miny)
				ny = miny;
			if (ny > maxy)
				ny = maxy;
			move(scr, mx, ny, !show_cursor, rapid_mode);
		} else if (config_input_match(ev, "word_down")) {
			int step = config_get_int("normal_large_step");
			int ny = my + step;
			const int miny = 1;
			const int maxy = sh - cursz;
