...
Install via [Homebrew](https://github.com/Yukaii/homebrew-tap) (builds from HEAD):
...
### This Fork (with Homebrew)

The easiest way to install this fork with all enhancements (ripple effects, horizontal scrolling, etc.):

```bash
brew install yukaii/tap/warpd
```

This will build from the latest master branch and automatically set up the launchd service.

Uninstallation:

```bash
brew uninstall yukaii/tap/warpd
```
...
# Clone this fork
git clone https://github.com/Yukaii/warpd.git
cd warpd
...
| `U` | Large Step Up | Move cursor up by a large step (`normal_large_step`) |
| `I` | Large Step Down | Move cursor down by a large step (`normal_large_step`) |
